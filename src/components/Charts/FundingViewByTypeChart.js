...
export default class FundingViewByTypeChart extends Component {
    ...
    render() {
        if (!_.isEmpty(this.props.data)) {
            ...
            return (
                <div>
                    <Line data={fundingViewChartData(this.props.data, this.props.selProj)} options={fundingViewChartOptions()} />  
                </div>
            );
        } else {
            return <div></div>;
...
// -----------------------------------------------------------------------------
function fundingViewChartData(apiData, selProj) {
    ...
    for (var i = 0; i < apiData.by_type.length; i++) { // for each entry in apiData.sow
        someDate = dateHelper(apiData.by_type[i].mo, apiData.by_type[i].yr); // mo/yr of some entry in funding result in API
