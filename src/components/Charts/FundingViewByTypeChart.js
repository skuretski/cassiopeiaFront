...
export default class FundingViewByTypeChart extends Component {
    ...
    render() {
        if (!_.isEmpty(this.props.data)) {
            ...
            var chartInfo = this.fundingViewChartInfo(this.props.data, this.props.selProj);
    ...
    // -----------------------------------------------------------------------------
    fundingViewChartInfo(apiData, selProj) {
        ...
        if (selProj == -1) {
            ...
            for (var i = 0; i < apiData.by_type.length; i++) {
                if (apiData.by_type[i].project_id == selProj) {
                    beginMo = apiData.by_type[i].mo;
                    beginYr = apiData.by_type[i].yr;
                    break;
                }
            }
            for (var i = apiData.by_type.length - 1; i >= 0; i--) {
                if (apiData.by_type[i].project_id == selProj) {
                    endMo = apiData.by_type[i].mo;
                    endYr = apiData.by_type[i].yr;
                    break;
                }
            }
        }
        ...
        // -----------------------------------------------------------------------------
        chartData.datasets = [];
        var dsToTypeMap = new Object();
        for (var i = 0; i < apiData.type.length; i++) {
            var dataset = new Object();
            dataset.label = 'Type: ' + apiData.type[i].title;
            dsToTypeMap[apiData.type[i].id] = i;
        ...
        // -----------------------------------------------------------------------------
        // Get actual funding data (for each funding type) to be plotted into chart datasets
        // -----------------------------------------------------------------------------
        var someDate;
        var j = 0;
        for (var i = 0; i < apiData.by_type.length; i++) { // for each entry in apiData.sow
            if (selProj == -1 || apiData.by_type[i].project_id == selProj) {
                someDate = this.dateHelper(apiData.by_type[i].mo, apiData.by_type[i].yr); // mo/yr of some entry in funding result in API
                while (someDate != chartData.labels[j]) {
                    //console.log('j = ' + j);
                    j++;
                }
                if (selProj == -1 || selProj == apiData.by_type[i].project_id) {
                    chartData.datasets[dsToTypeMap[apiData.by_type[i].fundingType_id]].data[j] += apiData.by_type[i].funding_amt;
                }
            }
        }
        dsToTypeMap = null; // i'm going to unshift into the dataset below, which will break the map
        // -----------------------------------------------------------------------------
